/* RC_SeeedMS_claw_test
control SparkFun robot arm (https://www.sparkfun.com/products/11524 & https://www.sparkfun.com/products/11674)
with
Seeed motor shield to drive servos (http://www.seeedstudio.com/wiki/Motor_Shield_V1.0)
*/
...
const unsigned long RCsigMidHigh = 1500;  // for setting dead zone in middle so no drift if joystick "centered"
const unsigned long RCsigMidLow = 1430;
...
void loop() {
  ...
  if (wristsig > RCsigMidHigh) {
    ...
    angledelta =  map(wristsig, RCsigMidHigh, RCsigMax, DeltaMax, DeltaMin);
    wristangle = wristangle + angledelta;
  }
  else if (wristsig < RCsigMidLow) {
    angledelta = map(wristsig, RCsigMin, RCsigMidLow, DeltaMin, DeltaMax);
    wristangle = wristangle - angledelta;
  }
